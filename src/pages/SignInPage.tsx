...
import { apiSignIn } from '../hooks/useApartments'
...
export default function SignInPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!user) { setError('Email немесе құпиясөз қате'); setLoading(false); return }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#fff', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        <div style={{ color: '#fff', fontWeight: 800, fontSize: 24 }}>Қош келдіңіз</div>
        <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: 14, marginTop: 6 }}>Аккаунтыңызға кіріңіз</div>
        <div style={{
      ...
      <div style={{ flex: 1, padding: '32px 28px 40px' }}>
        ...
        <div style={{ textAlign: 'center', fontSize: 14, color: '#888', marginTop: 24 }}>
          Аккаунтыңыз жоқ па?{' '}
          <Link to="/signup" style={{ color: '#D4AF37', fontWeight: 700, textDecoration: 'none' }}>Тіркелу</Link>
        </div>
      </div>
    </div>
  )
